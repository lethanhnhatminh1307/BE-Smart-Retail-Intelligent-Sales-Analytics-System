...
class AccountController {
  ...
  register(req, res, next) {
    ...
    Account.findOne({ userName: data.userName }, function (err, adventure) {
      if (!adventure) {
        ...
        const newAccount = new Account({ ...data, password: hash });
        newAccount
          .save()
  ...
  async registerManager(req, res, next) {
    ...
    const uppercase = password
      .split("")
      .some((item) => item.toUpperCase() === item);
    ...
    if (!(length && uppercase && number && speacial))
      res.status.json({
        success: false,
    ...
    if (account.password !== yourPassword)
      return res.status(200).json({
        success: false,
    ...
    Account.findOne({ userName: name })
      .then((data) => {
        if (!data)
          return res
            .status(200)
            .json({ check: false, message: "Sai tài khoản hoặc mật khẩu" });
        if (data?.disable)
          return res
            .status(200)
            .json({ check: false, message: "Tài khoản đã bị vô hiệu hóa" });
        ...
        const accessToken = jwt.sign(dataClient, accessTokenSecret, {
          expiresIn: "30000000000s",
        });
  ...
  logout(req, res, next) {
    ...
    Account.findOne({ userName: userName }).then((data) => {
    ...
    Account.findOne({ _id: id, userName: userName })
      .then((data) => {
        if (data) {
          ...
          res
            .status(403)
            .json({ message: "check id or userName", title: "error" });
        }
      })
      .catch(next);
  }
  ...
  updateInfoOfUser(req, res, next) {
    ...
    objectFields = avatar
      ? {
          ...objectFields,
          avatar: `http://${serverName}:${serverPort}/product/open-image?image=${avatar}`,
        }
      : { ...objectFields };
    objectFields = phoneNumber
      ? { ...objectFields, phoneNumber }
      : { ...objectFields };
    objectFields = birthday
      ? { ...objectFields, birthday }
      : { ...objectFields };
  ...
  async createAccountForEmployee(req, res, next) {
    ...
    data
      .save()
      .then((data) =>
        res.status(200).json({ title: "success", success: true, data }),
      )
      .catch(next);
  }
  ...
  async getAccountEmployee(req, res, next) {
    try {
      ...
      let datas;
      if (role.toUpperCase() === "ALL" && name === "")
        datas = await Account.find({
          role: { $not: { $regex: "manager" } },
          disable: isDisabled,
        });
      else if (role.toUpperCase() === "ALL" && name !== "")
        datas = await Account.find({
          role: { $not: { $regex: "manager" } },
          disable: isDisabled,
          $or: [
            {
              _id: mongoose.Types.ObjectId.isValid(name)
  ...
  async changePassword(req, res, next) {
    try {
      ...
      const account = await Account.updateOne(
        { userName, _id: userID, password: oldPassword },
      ...
      if (account.matchedCount && !account.modifiedCount)
        return res.status(200).json({
          success: false,
          title: "error",
